...
#mpl.style.use('tableau-colorblind10') #error loading this style 20181008
...
def filterPointsinShape_PNNL(shape, points_sn, points_we, points_lat, points_lon, points_elev=None, buffer_distance=0.06, 
                        buffer_resolution=16, labels=['SN', 'EW', 'LAT', 'LONG_', 'ELEV']):
    """
    Filter for datafiles that can be used
    
    shape: (geometry) a geometric polygon or MultiPolygon
    points_sn: row grid index
    points_we: column grid index
    points_lat: (series) a series of latitude points in WGS84 projection
    points_lon: (series) a series of longitude points in WGS84 projection
    points_elev: (series) a series of elevation points in meters; default is None
    buffer_distance: (float64) a numerical multiplier to increase the geodetic boundary area
    ...
    # Intersection each coordinate with the region
    limited_list = []    
    for sn, we, lon, lat, elev in zip(points_sn[bound_filter], points_we[bound_filter], points_lon[bound_filter], points_lat[bound_filter], points_elev[bound_filter]):
        gpoint = point.Point(lon, lat)
        if gpoint.intersects(region):
            limited_list.append([sn, we, lat, lon, elev])
...
def treatgeoself(shapefile, NAmer, mappingfile=os.path.join(os.getcwd(), 'mappingfile.csv'), buffer_distance=0.06):
    ...
    #NAmer_datapoints = readShapefileTable(NAmer).rename(columns={'lat': 'LAT', 'long': 'LONG_'})
...
def treatgeoself_PNNL(shapefile, NAmer, mappingfile=os.path.join(os.getcwd(), 'mappingfile.csv'), buffer_distance=0.06):
    """
    TreatGeoSelf to some [data] lovin'!
    ...
    # read in the North American continental DEM points for the station elevations
    NAmer_datapoints = readShapefileTable(NAmer).rename(columns={'Lat': 'LAT', 'Long': 'LONG_', 'El': 'ELEV'})
    
    # generate maptable
    maptable = filterPointsinShape_PNNL(shape_mp, 
                                   points_sn = NAmer_datapoints.SN,
                                   points_we=NAmer_datapoints.WE,
                                   points_lat=NAmer_datapoints.LAT, 
                                   points_lon=NAmer_datapoints.LONG_,
                                   points_elev=NAmer_datapoints.ELEV,
                                   buffer_distance=buffer_distance, buffer_resolution=16, 
                                   labels=['SN','WE','LAT', 'LONG_', 'ELEV'])
    maptable.reset_index(inplace=True)
    maptable = maptable.rename(columns={'index': 'FID'})
    print(maptable.shape)
    print(maptable.head())
...
def mappingfileToDF_PNNL(mappingfile, colvar='all', summary=True):
    """
...
def renderValuesInPoints_c(vardf, vardf_dateindex, shapefile, outfilepath, plottitle, colorbar_label,
                         vmin=None,vmax=None,spatial_resolution=1/16, margin=0.5, gridcell_alpha=0.5, epsg=3857,
                         basemap_image='Canvas/World_Dark_Gray_Base', cmap='coolwarm', figsize=(2,2)):
    """
    A function to render the dynamics across gridded cell centroids on the spatial landscape
    ...
    crs={'init': 'epsg:{0}'.format(epsg)}
    cat=vardf.T.reset_index(level=[1,2]).rename(columns={'level_1': 'LAT', 'level_2': 'LONG_'})
    geometry = cat.apply(lambda x: shapely.ops.transform(m, box(x['LONG_']-midpt*.8, x['LAT']-midpt*.8, 
                                                                x['LONG_']+midpt*.8, x['LAT']+midpt*.8)), axis=1)
#    geometry = cat.apply(lambda x: shapely.ops.transform(m, point(x['LONG_'], x['LAT']).buffer(midpt*.8)), axis=1)
    ...
    patches = []
    for ind, eachpol in cat.iterrows():
        patches.append(PolygonPatch(eachpol['geometry'], linewidth=0, zorder=10.0,
                                    fc=color_producer.to_rgba(eachpol[vardf_dateindex])))
