...
def wget_x_download_spSubset_PNNL_old(fileurl, 
                                 spatialbounds, 
                                 file_prefix='sp_', 
...
def wget_x_download_spSubset_PNNL(fileurl,
        ...
        try:
            ...
            if ping.getcode() != 404:
                ...
                # slice by the bounding box NOTE:dataframe slice includes last index
                ds=ds.assign_coords(SN=ds.SN, WE=ds.WE) # this line keeps index from reseting in cropped dataset
                spSubset = ds.sel(WE=slice(spatialbounds['minx'], spatialbounds['maxx']),
                                  SN=slice(spatialbounds['miny'], spatialbounds['maxy']))
                print('cropped')
                ...
                spSubset['TIME']=pd.DatetimeIndex(hour) #replaces Time index with datetime values, need for xray.concatenate function
...
def wget_x_download_spSubset_PNNL_dir(fileurl,
                                  filedate,
                                  spatialbounds,
                                  time_resolution='H',
                                  time_steps=24,
        ...
        Creates list of outputfile directories without downloading file
        """
...
def get_x_hourlywrf_PNNL2018_old(homedir,
                               spatialbounds,
...
def get_x_hourlywrf_PNNL2018_dir(homedir,
                             spatialbounds,
                             subdir='PNNL2018/Hourly_WRF_1981_2015/SaukSpatialBounds',
                             nworkers=4,
                             start_date='2005-01-01',
    ...
    NetCDFs=[]
    for url, date in zip(filelist, dates):
        NetCDFs.append(da.delayed(wget_x_download_spSubset_PNNL_dir)(fileurl=url,
                                                                 filedate=date,
                                                                 time_resolution=time_resolution,
                                                                 time_steps=time_steps,
                                                                 spatialbounds=spatialbounds,
...
def netcdf_to_ascii_PNNL2018(homedir, subdir, netcdfs, mappingfile, catalog_label, meta_file):
    # initialize list of dataframe outputs
    outfiledict = {}
